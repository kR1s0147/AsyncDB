...
impl UPDATE {
    ...
    pub fn read(header: Header, mut bytes: Bytes) -> Result<UPDATE, Error> {
        let header = Header::parse_header(bytes.iter())?;
        bytes.advance(3);
        if bytes.len() < (header.length as usize - 3) {
            return Err(Error::new("Not enough data to read UPDATE command"));
        }
        ...
        Ok(UPDATE::new(key_length, key, value_length, value, ttl))
    }
    ...
    pub fn write(&self, src: &mut BytesMut) -> Result<(), Error> {
        let size = self.size();
        let mut bytes = src;
        bytes.put_u8(0); // Command type for UPDATE
